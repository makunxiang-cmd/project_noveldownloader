...
- `infrastructure/` 各子模块之间尽量相互独立，仅依赖 `core/`
- `application/` 通过 `core/` 定义的 `Protocol` 接口消费基础设施（依赖倒置）
- `interfaces/`（`cli/` 与 `web/`）位于最外层，可 import 任何层，但不被任何层 import
...
| **P1 MVP：下载 + 转换** | `core` / `rules` / `fetchers(http)` / `parsers` / `converters` / `services(download+convert)` / `cli(download+convert)` + 2 条内置规则 + 契约测试 + 中文 i18n | 1-2 周 | `ndl download <url> -o book.epub` 端到端工作 |
...
首次 `ndl download` 或 `ndl serve` 展示免责声明并要求用户确认（`~/.ndl/config.toml` 写入 `disclaimer_accepted_at`）。CI 禁止功能变更绕过该步骤。
