...
- `scripts/verify_distribution.py` for repeatable wheel/sdist resource and metadata checks before release review

P6 and P7 are complete. P6.1 added the optional browser fetcher foundation,
P6.2 added browser-specific rule controls, P6.3 added browser runtime
diagnostics, P6.4 added release hardening docs plus artifact verification, and
P7 added the distribution verifier, release notes, install smoke strategy, and
release execution gate. A separate pre-release hardening pass
(`docs/superpowers/plans/2026-05-13-ndl-p7-pre-release-hardening.md`) landed
before v0.1.0 with nine reliability fixes:
...
For release-specific preflight, artifact inspection, install smoke, and
maintainer-only publication steps, see
...
2. The release-prep plan (`docs/superpowers/plans/2026-05-13-ndl-release-prep.md`)
3. The completed P7 plan (`docs/superpowers/plans/2026-05-13-ndl-p7-release-candidate.md`)
4. `AGENTS.md`

`git log --oneline` is the authoritative record of committed work. New changes
should start from a clean branch off `main`; inspect with `git status --short`
before editing and do not discard user changes.

Release-mutating work remains maintainer-only: do not bump final release
versions, tag, publish to GitHub Releases, or upload to PyPI without explicit
maintainer approval in the same turn.
