...
def test_verify_distribution_accepts_complete_artifacts(tmp_path: Path, capsys) -> None:
    wheel = tmp_path / "ndl-0.1.0.dev0-py3-none-any.whl"
    sdist = tmp_path / "ndl-0.1.0.dev0.tar.gz"
...
def _write_wheel(path: Path, *, members: set[str]) -> None:
    ...
    with zipfile.ZipFile(path, mode="w") as archive:
        ...
        archive.writestr("ndl-0.1.0.dev0.dist-info/METADATA", metadata)
...
def _write_sdist(path: Path, *, suffixes: set[str]) -> None:
    with tarfile.open(path, mode="w:gz") as archive:
        for suffix in suffixes:
            ...
            archive.add(temp, arcname=f"ndl-0.1.0.dev0/{suffix}")
